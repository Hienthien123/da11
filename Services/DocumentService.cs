...
namespace Services
{
    ...
    public class DocumentService : IDocumentService
    {
        ...
        {     
            var data = new List<TblDocument>();
            if(statusCode == 5)
            {
                var approvalData = await _documentApprovalRepository.GetMulti(x => x.StatusCode == 5);
                if(approvalData.Count > 0)
                {
                    data = (await _documentRepository.GetMulti(x => approvalData.Select(y => y.DocId).Contains(x.Id) && (x.StatusCode == 4 || x.StatusCode == 5)));
                }
            }
            else
            {
                data = (await _documentRepository.GetMulti(x => x.StatusCode == statusCode)).OrderByDescending(x => x.Id).ToList();            
            }
            if (data == null)
            {
        ...
        {
            var docIds = _documentRepository.Query(x => (x.StatusCode == 6 || x.StatusCode == 7)).Select(x => x.Id).ToList();
            foreach (var docId in docIds) 
            {
                var commentsData = _commentRepository.Query(x => x.DocId == docId);
                var approvalData = _documentApprovalRepository.Query(x => x.DocId == docId);
                _commentRepository.RemoveRange(commentsData);
                _documentApprovalRepository.RemoveRange(approvalData);
                await _commentRepository.SaveChanges();
                await _documentApprovalRepository.SaveChanges();
            }
        }

        private async Task UpdateOverdueStatus()
        {
            var overdueList = _documentRepository.Query(x => (x.StatusCode == 3) && (x.DateEndApproval != null && DateTime.Compare(DateTime.Today, (DateTime)x.DateEndApproval) > 0)).ToList();
