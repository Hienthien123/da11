...
namespace Services
{
    ...
    public class DocumentApprovalService : IDocumentApprovalService
    {
        ...
        public async Task<List<DocumentApprovalSummaryDTO>> GetApprovalSummary(int id)
        {
            ...
            var approverRoles = await roleRepository.GetMulti(x => x.RoleName.ToLower() == "Approver".ToLower() || x.RoleName.ToLower() == "General Approver".ToLower());

            var approverRoleIds = new List<Guid>();
            if (approverRoles != null) approverRoleIds = approverRoles.Select(x => x.RoleId).ToList();

            var approverIds = (await userInRoleRepository.GetMulti(x => approverRoleIds.Contains(x.RoleId))).Select(x => x.UserId);
            ...
            {
                var endAtDate = (DateTime)document.DateEndApproval!;
                var documentApprovals = await documentApprovalRepository.GetMulti(x => x.DocId == document.Id);
                if(documentApprovals.Count > 0)
                {
                    endAtDate = (DateTime)documentApprovals[documentApprovals.Count - 1].Created;
                }
                var yesIds = documentApprovals.Where(x => x.StatusCode == 4 || x.StatusCode == 5).Select(x => x.UserId);
                var noIds = documentApprovals.Where(x => x.StatusCode == 6).Select(x => x.UserId);
