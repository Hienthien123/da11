...
@Component({
  selector: 'app-form-chucvu',
  templateUrl: './form-chucvu.component.html',
  styleUrls: ['./form-chucvu.component.css']
})
export class FormChucvuComponent implements OnInit {
    ...
    if (!checkForm) {         
      if(this.id){
        this.fieldService.update(this.dataField).subscribe(res => {
          if (res.isSuccess) {            
            this.toastr.success('Chỉnh sửa thông tin chức vụ thành công');
            this.router.navigate(['/admin/chuc-vu'])
          } else {
