...
export class DocumentFormComponent implements OnInit {
  ...
  submit(sendToApprove: boolean) {       
    ...
    var checkForm = this.data.title?.trim() == '' || this.data.fieldId == 0 || this.data.note?.trim() == '' || this.files.length == 0;
    if(!checkForm){
      if(sendToApprove){
          ...
          const addDataPromise = this.documentService.create(this.data, this.files).subscribe((res) => {
            if (res.isSuccess) {     
              ...
              if(this.data.statusCode == StatusCode.Pending){
                ...
                this.notificationService.SendSMS(res.result.id, 1).subscribe(res => {
                  console.log(res);                  
                  if(res.isSuccess){
