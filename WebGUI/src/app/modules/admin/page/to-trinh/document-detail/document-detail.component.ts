...
export class DocumentDetailComponent implements OnInit {
  ...
  handleAction(status: number){
    ...
    if(status == 4){
      ...
      message = `Xác nhận duyệt tờ trình ${this.document?.title}, phản hồi của bạn sẽ được đính kèm vào phần ý kiến trao đổi`
      const modalRef = this.modalService.open(CommentComponent);
      modalRef.componentInstance.title = 'Ý kiến bổ sung';
      modalRef.result.then(result => {
        console.log(result);
        
        response = '[Duyệt kèm ý kiến bổ sung] ' + result
        this.submitApproval(status, response, message)
      })
    } else if (status == 6){
  ...
  handleAction_General(status: number){
    ...
    if(status == 4){
      ...
      message = `Xác nhận trả lại tờ trình ${this.document?.title}? Hành động này sẽ bỏ qua ý kiến của các cán bộ khác và lập tức trả lại tờ trình`;      
    }
  ...
  showCommentDialog(comment?: string){
