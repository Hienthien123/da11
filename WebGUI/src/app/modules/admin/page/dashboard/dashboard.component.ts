  ...
  getData(){
    this.documentService.getAll().subscribe(res => {
      if(res.isSuccess){
        ...
        this.dashboardInfo.all = this.documents.filter(x => x.statusCode != 2).length;        
        ...
        this.dashboardInfo.pendingCount = this.documents.filter(x => x.statusCode == 3 || x.statusCode == 7).length;          
      }
    })
  }  
