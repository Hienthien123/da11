...
builder.Services.AddDbContext<QLTTrContext>(option => {
    option.UseSqlServer(builder.Configuration.GetConnectionString("QLChuKySoSTTTTConnectionString"));
});
...
builder.Services.AddCors(options =>
{
    options.AddDefaultPolicy(
        policy =>
        {
            policy.WithOrigins("https://totrinh.yenbai.net.vn", "https://totrinh.yenbai.net.vn/", "http://localhost:4200").AllowAnyOrigin().AllowAnyHeader().AllowAnyMethod();
        });
});
